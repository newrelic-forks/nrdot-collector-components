...
func (s *CoreScraper) scrapeSGASharedPoolLibraryCacheReloadRatioMetrics(ctx context.Context, now pcommon.Timestamp) []error {
	...
	for _, metric := range metrics {
		...
		s.mb.RecordOracledbSgaSharedPoolLibraryCacheReloadRatioDataPoint(now, metric.Ratio, instanceID)
		metricCount++
	}

	s.logger.Debug("SGA shared pool library cache reload ratio metrics scrape completed")
...
func (s *CoreScraper) scrapeSGASharedPoolLibraryCacheHitRatioMetrics(ctx context.Context, now pcommon.Timestamp) []error {
	...
	for _, metric := range metrics {
		instanceID := getInstanceIDString(metric.InstID)
		s.mb.RecordOracledbSgaSharedPoolLibraryCacheHitRatioDataPoint(now, metric.Ratio, instanceID)
		metricCount++
	}

	s.logger.Debug("SGA shared pool library cache hit ratio metrics scrape completed")
...
func (s *CoreScraper) scrapeSGASharedPoolDictCacheMissRatioMetrics(ctx context.Context, now pcommon.Timestamp) []error {
	...
	for _, metric := range metrics {
		instanceID := getInstanceIDString(metric.InstID)
		s.mb.RecordOracledbSgaSharedPoolDictCacheMissRatioDataPoint(now, metric.Ratio, instanceID)
		metricCount++
	}

	s.logger.Debug("SGA shared pool dictionary cache miss ratio metrics scrape completed")
...
func (s *CoreScraper) scrapeSGALogBufferSpaceWaitsMetrics(ctx context.Context, now pcommon.Timestamp) []error {
	...
	for _, metric := range metrics {
		instanceID := getInstanceIDString(metric.InstID)
		s.mb.RecordOracledbSgaLogBufferSpaceWaitsDataPoint(now, metric.Count, instanceID)
		metricCount++
	}

	s.logger.Debug("SGA log buffer space waits metrics scrape completed")
...
func (s *CoreScraper) scrapeSGALogAllocRetriesMetrics(ctx context.Context, now pcommon.Timestamp) []error {
	...
	for _, metric := range metrics {
		...
		s.mb.RecordOracledbSgaLogAllocationRetriesRatioDataPoint(now, ratioValue, instanceID)
		metricCount++
	}

	s.logger.Debug("SGA log allocation retries metrics scrape completed")
...
func (s *CoreScraper) scrapeSGAHitRatioMetrics(ctx context.Context, now pcommon.Timestamp) []error {
	...
	for _, metric := range metrics {
		...
		s.mb.RecordOracledbSgaHitRatioDataPoint(now, ratioValue, instanceID)
		metricCount++
	}

	s.logger.Debug("SGA hit ratio metrics scrape completed")
