...
import (
	...
	"github.com/newrelic/nrdot-collector-components/receiver/newrelicoraclereceiver/scrapers"
)
...
func (s *newRelicOracleScraper) executeQPMScrapers(ctx context.Context, errChan chan<- error) {
	...
	if len(waitEventSQLIdentifiers) > 0 {
		...
		updatedIdentifiers, childCursorErrs := s.childCursorsScraper.ScrapeChildCursorsForIdentifiers(ctx, waitEventSQLIdentifiers, s.config.ChildCursorsPerSQLID)
